...
# data structure
trianglePeriodicity = 4 # quarterly. Note this is hardcoded for exposure as 
                        # quarterly for now, so use 4. Needs further work otherwise.
triangleDimension = 16 # 16 -> 4 years at 4 periods per year
dataStartDate = as.Date(paste0(year(valuationDate) - triangleDimension/trianglePeriodicity, '-12-31')) 
# to trim the data. Note we produce 2 extra prior years and trim due to run-on.
...
set_parameters(ref_claim = 20000, time_unit = 1/trianglePeriodicity) 
# used for some functions, not sure exactly which

# define Poisson params for claim freq
poissonMean = numPolicies*claimFreq/trianglePeriodicity
frequency = claim_frequency(I = triangleDimension + 2*trianglePeriodicity, 
                            simfun = rpois, lambda = poissonMean) # poisson distribution
...
size = claim_size(frequency, simfun = rlnorm, 
                  meanlog = lnormMu, sdlog = lnormSigma) # lognormal distribution
...
notification = claim_notification(frequency, size, 
                                  rfun = rweibull, paramfun = weibullParams)
...
paymentInflation = claim_payment_inflation(frequency, paymentSize, 
                                           paymentTime, occurrence, size, 
                                           baseInflation, superInflationOccurrence, 
                                           superInflationPaymentTime)

claimsFull = claims(frequency, occurrence, size, notification, 
                    closure, paymentNo, paymentSize, paymentDelay, paymentTime, 
                    paymentInflation)
...
############### large claims, long settlement period, low frequency  ###########
...
set_parameters(ref_claim = 2000000, time_unit = 1/trianglePeriodicity) 
# used for some functions, not sure exactly which
...
# define Weibull params for closure
weibullMean = 10 # mean number of periods delay
weibullSD = 6 # std dev around this mean
weibullParams = function(claim_size, occurrence_period){
  weibullCoV = weibullSD/weibullMean
  shape = get_Weibull_parameters(weibullMean, weibullCoV)[1, ]
  scale = get_Weibull_parameters(weibullMean, weibullCoV)[2, ]
...
# define geometric params for payment numbers
geomMean = 5 # mean number of payments per claim
geomParams = function(claim_size){
  geomP = 1/geomMean
  c(prob = geomP)
}
...
claims_Large = generate_claim_dataset(frequency, occurrence, size, notification, closure, paymentNo) %>% 
  data.table() %>% 
  setorder(occurrence_time) %>% 
  .[, class := class] %>% 
  .[, resgroup := resgroup]
...
set_parameters(ref_claim = 10000, time_unit = 1/trianglePeriodicity) 
# used for some functions, not sure exactly which
...
# define Weibull params for closure
weibullMean = 3 # mean number of periods delay
weibullSD = 2 # std dev around this mean
weibullParams = function(claim_size, occurrence_period){
  weibullCoV = weibullSD/weibullMean
  shape = get_Weibull_parameters(weibullMean, weibullCoV)[1, ]
  scale = get_Weibull_parameters(weibullMean, weibullCoV)[2, ]
...
claims_Small = generate_claim_dataset(frequency, occurrence, size, notification, closure, paymentNo) %>% 
  data.table() %>% 
  setorder(occurrence_time) %>% 
  .[, class := class] %>% 
  .[, resgroup := resgroup]
